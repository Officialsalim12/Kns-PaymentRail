  ...
  return authAsync.when(
    data: (user) async {
      ...
      final completedPayments =
          await dataSource.getPaymentsByStatus(organizationId, 'completed');
      ...
      final pendingMembers =
          members.where((m) => m['status'] == 'pending').take(10).toList();
...
final adminActiveMembersProvider =
    FutureProvider<List<Map<String, dynamic>>>((ref) async {
  final authAsync = ref.watch(authProvider);
  final dataSource = ref.read(supabaseDataSourceProvider);
...
final adminUsersProvider =
  ...
  return authAsync.when(
    data: (user) async {
      ...
      final organizationId = userProfile['organization_id'];
      return await dataSource.getOrganizationUsers(organizationId);
    },
    loading: () => [],
    error: (_, __) => [],
  );
...
final adminActivityLogsProvider =
  ...
  return authAsync.when(
    data: (user) async {
      ...
      final organizationId = userProfile['organization_id'];
      return await dataSource.getActivityLogs(organizationId);
    },
    loading: () => [],
    error: (_, __) => [],
  );
...
final adminTabsProvider =
  ...
  return authAsync.when(
    data: (user) async {
      ...
      final organizationId = userProfile['organization_id'];
      return await dataSource.getOrganizationTabs(organizationId);
    },
    loading: () => [],
    error: (_, __) => [],
  );
