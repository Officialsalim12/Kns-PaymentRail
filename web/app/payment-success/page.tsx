...
function PaymentSuccessContent() {
  ...
  useEffect(() => {
    ...
    const handlePaymentSuccess = async () => {
      try {
        ...
        // Only set a dashboard path if the user is actually logged in
        let determinedPath = null
        ...
        // Only auto-redirect if we have a valid dashboard path for a logged-in user
        if (determinedPath) {
          redirectTimerRef.current = setTimeout(() => {
            router.push(determinedPath)
          }, 5000) // Increased to 5s to allow reading the success message
        }
      } catch (error) {
        console.error('Error handling payment success:', error)
        // On error, we don't auto-redirect to avoid loops or confusing behavior
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] p-4 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-emerald-50 rounded-full blur-[100px] opacity-60" />
      ...
      <div className="w-full max-w-xl relative z-10 transition-all duration-1000">
        <div className="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-white/20 backdrop-blur-sm overflow-hidden transition-all duration-700 hover:shadow-[0_40px_80px_rgba(0,0,0,0.08)]">
          ...
          {/* Header Section */}
          <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 p-12 text-white text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-emerald-50/90 text-lg font-medium">Thank you for your transaction with Fundflow</p>
            </div>
          </div>
          ...
          <div className="p-10 text-center">
            {syncing ? (
              ...
              <div className="space-y-8 transition-all duration-700">
                <div className="bg-slate-50/80 rounded-3xl p-8 border border-slate-100 group transition-all hover:bg-white hover:shadow-lg hover:shadow-slate-100/50 text-left">
                  <div className="flex items-center justify-between mb-6">
                    <span className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Order Reference</span>
                ...
                <div className="space-y-4 pt-2">
                  ...
                  >
                    {dashboardPath === '/admin' || dashboardPath === '/member' ? 'Go to Dashboard' : 'Return to Login'}
                  </button>
                  
                  {dashboardPath && (dashboardPath === '/admin' || dashboardPath === '/member') && (
                    <div className="flex items-center justify-center gap-2">
                      <Loader2 className="h-4 w-4 animate-spin text-slate-300" />
                      <p className="text-sm text-slate-400 font-medium">
                        Automatic redirect in <span className="text-slate-500 font-bold">3 seconds</span>
                      </p>
                    </div>
                  )}
                </div>
