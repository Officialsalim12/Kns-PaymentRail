  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    if (insertError.code === '23503' || insertError.message?.includes('foreign key')) {
      try {
        ...
        if (getUserError) {
          console.error(`[Organization Registration] Error fetching auth user (attempt ${attempt}):`, getUserError)
        ...
        if (authUser && authUser.user) {
          ...
          if (!authUser.user.email_confirmed_at) {
            try {
              ...
              if (confirmError) {
                console.error(`[Organization Registration] Error confirming email:`, confirmError)
              } else {
                // Wait a bit after confirming email before retrying
                await new Promise(resolve => setTimeout(resolve, 500))
              }
            } catch (confirmErr) {
              console.error(`[Organization Registration] Exception confirming email:`, confirmErr)
            }
          }
        ...
        console.error(`[Organization Registration] Exception checking auth user (attempt ${attempt}):`, err)
