...
export async function POST(request: NextRequest) {
    try {
        ...
        const redirectUrl = new URL('/payment-success', base);
        console.log(`[payment-success] Redirecting to: ${redirectUrl.toString()}`);
        ...
        return NextResponse.redirect(new URL('/payment-success', base), 303);
    }
}
...
export async function GET(request: NextRequest) {
    try {
        ...
        const base = getBaseUrl(request);
        const redirectUrl = new URL('/payment-success', base);
        console.log(`[payment-success GET] Redirecting to: ${redirectUrl.toString()}`);
        ...
        console.error('Error handling GET request to payment-success:', error);
        const base = process.env.NEXT_PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_APP_URL || PRODUCTION_BASE_URL;
        return NextResponse.redirect(new URL('/payment-success', base), 303);
    }
}
