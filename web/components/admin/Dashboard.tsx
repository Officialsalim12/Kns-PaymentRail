    ...
    <div className="space-y-8 pb-8">
              ...
              <div className="flex flex-col sm:flex-row items-center text-center sm:text-left gap-6">
                {organization.logo_url ? (
                  <img
                    ...
                    className="relative h-20 w-20 sm:h-24 sm:w-24 object-contain rounded-2xl border-2 border-white/30 bg-white/10 backdrop-blur-sm shadow-xl"
                  />
                ) : (
                  <div className="relative h-20 w-20 sm:h-24 sm:w-24 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center border-2 border-white/30 shadow-xl">
                    <span className="text-2xl sm:text-3xl font-bold text-white uppercase tracking-wider">
                      {getOrganizationAbbreviation(organization.name)}
                    </span>
                  </div>
                )}
                <div className="space-y-1 text-center sm:text-left">
                  <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight leading-tight">
                    {organization.name}
                  </h1>
                </div>
              </div>

              <div className="flex flex-col items-center sm:items-end gap-6 w-full lg:w-auto">
                <button
                  onClick={() => setShowBulkTabCreator(true)}
                  className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 py-3.5 bg-white text-primary-700 rounded-2xl hover:bg-primary-50 transition-all shadow-xl hover:shadow-2xl font-bold text-sm tracking-wide active:scale-95 group"
                >
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6">
          <div className="flex items-center justify-between mb-5">
            <div className="p-3.5 bg-primary-500 rounded-xl shadow-md">
              <Wallet className="h-6 w-6 text-white" />
            </div>
            {revenueChange !== 0 && (
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold ${revenueChange >= 0 ? 'text-green-600 bg-green-50 border-green-100' : 'text-red-600 bg-red-50 border-red-100'}`}>
        ...
        <Link href="/admin/payments" className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:border-green-200 transition-colors">
          <div className="flex items-center justify-between mb-5">
            <div className="p-3.5 bg-green-500 rounded-xl shadow-md">
        ...
        <Link href="/admin/members" className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:border-blue-200 transition-colors">
          <div className="flex items-center justify-between mb-5">
            <div className="p-3.5 bg-blue-500 rounded-xl shadow-md">
        ...
        <Link href="/admin/approvals" className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:border-orange-200 transition-colors">
          <div className="flex items-center justify-between mb-5">
            <div className="p-3.5 bg-orange-500 rounded-xl shadow-md">
          ...
          <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Pending Approvals</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-8">
          <h2 className="text-xl font-bold text-gray-900 mb-6">Revenue Performance</h2>
          <div className="h-64 sm:h-72 flex items-end justify-between gap-1.5 sm:gap-4">
            {chartData.map((day, index) => (
              <div key={index} className="flex-1 flex flex-col items-center gap-3 group">
                <div className="w-full bg-gradient-to-t from-primary-600 to-primary-400 rounded-t-xl transition-all duration-300 group-hover:from-primary-700 group-hover:to-primary-500 relative cursor-pointer"
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-8">
        <h2 className="text-xl font-bold text-gray-900 mb-8 text-center md:text-left">Member Payment Status</h2>
          ...
          <div className="flex flex-col items-center">
            <div className="relative w-48 h-48 sm:w-56 sm:h-56 mb-6">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 200 200">
                ...
                {stats.totalMembers > 0 && (
                  <>
                    <circle cx="100" cy="100" r="80" fill="none" stroke="#10b981" strokeWidth="24"
                      strokeDasharray={`${(stats.paidMembers / stats.totalMembers) * 502.65} 502.65`}
                      strokeLinecap="round" />
                    {stats.unpaidMembers > 0 && (
                      <circle cx="100" cy="100" r="80" fill="none" stroke="#ef4444" strokeWidth="24"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-4xl font-extrabold text-gray-900">{stats.totalMembers}</p>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Total</p>
              </div>
            </div>
            ...
            <div className="flex gap-8">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-gray-700">{stats.paidMembers} Paid</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-red-500" />
                <span className="text-sm font-bold text-gray-700">{stats.unpaidMembers} Unpaid</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="p-6 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-extrabold text-emerald-900">{stats.totalMembers > 0 ? ((stats.paidMembers / stats.totalMembers) * 100).toFixed(0) : 0}%</p>
              </div>
            ...
            <div className="p-6 rounded-2xl bg-red-50 border border-red-100 flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-red-600 uppercase tracking-widest mb-1">Outstanding Rate</p>
                <p className="text-3xl font-extrabold text-red-900">{stats.totalMembers > 0 ? ((stats.unpaidMembers / stats.totalMembers) * 100).toFixed(0) : 0}%</p>
              </div>
